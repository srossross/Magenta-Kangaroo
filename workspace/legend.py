...
from maka.gl_primitives.drop_shadow import drop_shadow
from maka.gl_primitives.frame_border import draw_frame_border

sys.path.append('/Users/sean/indigo_workspace/Magenta-Kangaroo')
from maka.util import bring_to_front, gl_enable, matrix, gl_begin
...
class Chooser(QtGui.QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("Legend Position")
        layout = QtGui.QGridLayout(self)
        layout.setSpacing(1)
        self.setLayout(layout)
        
        exp = QtGui.QSizePolicy.Expanding
        button_names = [["Top Left", "Top Right"], ["Bottom Left", "Bottom Right"]]
        self.b_map = {}
        self.group = group = QtGui.QButtonGroup(self)
        self.buttons = buttons = [[None, None], [None, None]]
        for i, vert in enumerate(button_names):
            for j, name in enumerate(vert):
                button = QtGui.QPushButton(name, self)
                group.addButton(button, i + 10 * j)
                self.b_map[button] = i + 10 * j
                buttons[i][j] = button
                button.setSizePolicy(exp, exp)
...
#                button.clicked.connect(self.button_pressed)
        ...
        group.buttonClicked.connect(self.button_pressed)
        buttons[0][1].setChecked(True)
    
    position_changed = QtCore.Signal(float, float)

    @QtCore.Slot(QtGui.QPushButton)
    def button_pressed(self, button):
        
        bid = self.b_map[button]
        top = float((bid % 10) == 0)
        right = float((bid // 10) != 0)

        self.position_changed.emit(top, right)

class QGL(QtOpenGL.QGLWidget):
    
    def __init__(self):
        super(QGL, self).__init__()
    ...
    def draw_box(self, rect, rrect):
        ...
        self.qglColor(QtCore.Qt.darkGray)
        GL.glLineWidth(1)
        rrect.draw_outline()
        ...
        for li in self.legend_items[::-1]:
            self.renderText(10., offset, 0.0, li.text, font)
    ...
    def paintGL(self):
        ...
        with matrix(GL.GL_PROJECTION):
            ...
            GL.glVertex2f(0, 0)
            GL.glVertex2f(vp.width(), vp.height())
            GL.glEnd()
        ...
        self.qglColor(self.palette().color(QtGui.QPalette.Dark))
        draw_frame_border(vp.width(), vp.height())
...
def main():
    ...
    chooser.position_changed.connect(widget.change_position)
    bring_to_front()
    app.exec_()
